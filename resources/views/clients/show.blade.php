    ...
    <div class="py-8">
        <div class="mx-auto max-w-7xl space-y-5 sm:px-6 lg:px-8">
            ...
            <section class="overflow-hidden bg-white p-6 shadow-sm sm:rounded-lg">
                <div class="flex flex-wrap items-end justify-between gap-3 border-b border-gray-100 pb-4"><div><h3 class="font-semibold text-gray-900">Customer 360</h3><p class="mt-1 text-sm text-gray-500">Ringkasan relasi komersial, delivery, dan dukungan pelanggan.</p></div><a href="{{ route('invoices.index', ['client' => $client->id]) }}" class="text-sm font-medium text-indigo-600 hover:text-indigo-800">Buka invoice</a></div>
                <div class="mt-5 grid grid-cols-2 gap-3 md:grid-cols-4 lg:grid-cols-6">
                    <div class="rounded-lg bg-indigo-50 p-4"><p class="text-xs font-medium text-indigo-700">Invoice</p><p class="mt-1 text-xl font-semibold text-indigo-950">{{ $client->invoices->count() }}</p><p class="mt-1 text-xs text-indigo-700">{{ number_format($client->invoices->sum('total'), 0, ',', '.') }} ditagihkan</p></div>
                    <div class="rounded-lg bg-emerald-50 p-4"><p class="text-xs font-medium text-emerald-700">Terbayar</p><p class="mt-1 text-xl font-semibold text-emerald-950">{{ number_format($client->invoices->sum('paid_amount'), 0, ',', '.') }}</p><p class="mt-1 text-xs text-emerald-700">penerimaan</p></div>
                    <div class="rounded-lg bg-amber-50 p-4"><p class="text-xs font-medium text-amber-700">Proposal</p><p class="mt-1 text-xl font-semibold text-amber-950">{{ $client->proposals->count() }}</p><p class="mt-1 text-xs text-amber-700">dokumen sales</p></div>
                    <div class="rounded-lg bg-sky-50 p-4"><p class="text-xs font-medium text-sky-700">Proyek</p><p class="mt-1 text-xl font-semibold text-sky-950">{{ $client->projects->count() }}</p><p class="mt-1 text-xs text-sky-700">delivery</p></div>
                    <div class="rounded-lg bg-rose-50 p-4"><p class="text-xs font-medium text-rose-700">Ticket</p><p class="mt-1 text-xl font-semibold text-rose-950">{{ $client->tickets->whereIn('status', ['open', 'in_progress'])->count() }}</p><p class="mt-1 text-xs text-rose-700">masih terbuka</p></div>
                    <div class="rounded-lg bg-violet-50 p-4"><p class="text-xs font-medium text-violet-700">Lead</p><p class="mt-1 text-xl font-semibold text-violet-950">{{ $client->convertedLeads->count() }}</p><p class="mt-1 text-xs text-violet-700">terkonversi</p></div>
                </div>
                <div class="mt-6 grid grid-cols-1 gap-5 lg:grid-cols-2">
                    <div><h4 class="text-sm font-semibold text-gray-900">Dokumen terbaru</h4><div class="mt-3 divide-y divide-gray-100 rounded-lg border border-gray-100">@forelse($client->invoices as $invoice)<a href="{{ route('invoices.show', $invoice) }}" class="flex items-center justify-between gap-3 px-4 py-3 hover:bg-gray-50"><span><span class="block font-medium text-gray-800">{{ $invoice->number }}</span><span class="block text-xs text-gray-500">{{ $invoice->status }} · {{ $invoice->date?->format('d M Y') }}</span></span><span class="text-sm font-semibold text-gray-700">{{ $client->currency }} {{ number_format($invoice->total, 0, ',', '.') }}</span></a>@empty<p class="px-4 py-5 text-sm text-gray-500">Belum ada invoice.</p>@endforelse</div></div>
                    <div><h4 class="text-sm font-semibold text-gray-900">Aktivitas terbaru</h4><div class="mt-3 divide-y divide-gray-100 rounded-lg border border-gray-100">@forelse($client->activityLogs as $activity)<div class="px-4 py-3"><p class="text-sm text-gray-800">{{ $activity->description }}</p><p class="mt-1 text-xs text-gray-500">{{ $activity->actor?->name ?? 'Sistem' }} · {{ $activity->created_at->format('d M Y H:i') }}</p></div>@empty<p class="px-4 py-5 text-sm text-gray-500">Aktivitas baru akan tercatat setelah perubahan dilakukan.</p>@endforelse</div></div>
                </div>
            </section>
