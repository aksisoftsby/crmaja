    ...
    <main class="relative min-h-screen overflow-hidden px-5 py-8 sm:px-8 lg:flex lg:items-center lg:justify-center lg:p-10">
        ...
        <div class="grid w-full max-w-6xl overflow-hidden rounded-[2rem] border border-white/80 bg-white/80 shadow-2xl shadow-slate-900/10 backdrop-blur lg:grid-cols-[0.95fr_1.05fr]">
            <section class="flex min-h-[560px] flex-col justify-center px-6 py-10 sm:px-12 lg:px-14">
                ...
                <section class="mt-5 rounded-2xl border border-teal-100 bg-teal-50/70 p-4" aria-label="Informasi login demo Client Portal"><div class="flex items-start justify-between gap-4"><div><h2 class="text-sm font-extrabold text-teal-950">Informasi login demo</h2><p class="mt-1 text-xs leading-5 text-teal-800">Klik akun untuk mengisi form. Akun ini memuat dokumen, project, dan ticket contoh satu perusahaan demo.</p></div><span class="rounded-full bg-white px-2.5 py-1 text-[11px] font-bold text-teal-700 shadow-sm">DEMO</span></div><button type="button" data-demo-account="client-portal" @click="$refs.email.value = 'client@aksisoft.test'; $refs.password.value = 'ClientPass123!'; $refs.email.focus()" class="mt-4 w-full rounded-xl border border-white bg-white/90 p-3 text-left text-xs transition hover:border-teal-400 hover:bg-teal-50"><span class="block font-extrabold text-slate-800">Client Portal</span><span class="mt-1 block break-all font-medium text-slate-600">client@aksisoft.test</span><span class="mt-1 block font-mono text-slate-700">ClientPass123!</span></button></section>
                </div>
                <p class="mt-5 max-w-md text-sm text-slate-600">Akun staf? <a href="{{ route('login') }}" class="font-bold text-teal-700 transition hover:text-teal-900">Masuk ke Aksi CRM</a></p>
            </section>
            <section class="relative hidden min-h-[640px] overflow-hidden bg-slate-950 p-12 text-white lg:flex lg:flex-col lg:justify-between"><div class="absolute inset-0 bg-[radial-gradient(circle_at_82%_16%,rgba(45,212,191,0.35),transparent_30%),radial-gradient(circle_at_20%_86%,rgba(20,184,166,0.24),transparent_35%)]"></div><div class="relative"><div class="inline-flex items-center gap-2 rounded-full border border-white/10 bg-white/[0.07] px-3.5 py-2 text-sm font-bold text-teal-100"><span class="h-2 w-2 rounded-full bg-teal-300"></span> Terhubung dengan layanan Anda</div><h2 class="mt-10 max-w-md text-4xl font-extrabold leading-tight tracking-tight">Dokumen dan progres, selalu dalam jangkauan.</h2><p class="mt-5 max-w-md text-base leading-7 text-slate-300">Gunakan Client Portal untuk meninjau setiap detail yang dibagikan tim Aksi CRM kepada perusahaan Anda.</p></div><div class="relative space-y-3"><div class="rounded-2xl border border-white/10 bg-white/[0.07] p-5 backdrop-blur"><p class="text-sm font-bold">Invoice & Proposal</p><p class="mt-1 text-sm leading-6 text-slate-300">Tinjau nilai, rincian item, serta riwayat pembayaran.</p></div><div class="rounded-2xl border border-white/10 bg-white/[0.07] p-5 backdrop-blur"><p class="text-sm font-bold">Project & Ticket</p><p class="mt-1 text-sm leading-6 text-slate-300">Pantau progres pekerjaan dan percakapan layanan Anda.</p></div></div></section>
        </div>
    </main>
</body>
</html>
