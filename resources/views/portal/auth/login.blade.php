    ...
    <main class="relative min-h-screen overflow-hidden px-5 py-8 sm:px-8 lg:flex lg:items-center lg:justify-center lg:p-10">
        ...
        <div class="grid w-full max-w-6xl overflow-hidden rounded-[2rem] border border-white/80 bg-white/80 shadow-2xl shadow-slate-900/10 backdrop-blur lg:grid-cols-[0.95fr_1.05fr]">
            <section class="flex min-h-[560px] flex-col justify-center px-6 py-10 sm:px-12 lg:px-14">
                ...
                <form method="POST" action="{{ route('portal.login.store') }}" class="mt-8 max-w-md space-y-5">@csrf
                    ...
                    <div><x-input-label for="email" value="Email" class="font-bold text-slate-700"/><x-text-input id="email" name="email" type="email" class="mt-2 block w-full rounded-xl border-slate-300 px-4 py-3 shadow-sm focus:border-teal-600 focus:ring-teal-600" :value="old('email')" placeholder="nama@perusahaan.com" required autofocus autocomplete="username"/></div>
                    <div><x-input-label for="password" value="Kata sandi" class="font-bold text-slate-700"/><x-text-input id="password" name="password" type="password" class="mt-2 block w-full rounded-xl border-slate-300 px-4 py-3 shadow-sm focus:border-teal-600 focus:ring-teal-600" placeholder="Masukkan kata sandi" required autocomplete="current-password"/></div>
