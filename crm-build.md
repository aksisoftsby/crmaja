...
- Polymorphic (`related_type` + `related_id`) dipakai bersama oleh: `notes`, `attachments`, `custom_field_values`, `taggables`, `reminders`, `calendar_events`, `tasks` — supaya satu struktur bisa nempel ke banyak jenis entity (leads, clients, projects, invoices, tickets, dll) tanpa perlu tabel